...
from .fixtures import SIMPLE_HTML, COMPLEX_HTML
...
class TestCLI:
    ...
    def test_cli_compound_selector(self):
        ...
        result = self.runner.invoke(main, ['--no-color', '.content p'], input=SIMPLE_HTML)
        assert result.exit_code == 0
        assert 'First paragraph' in result.output
        assert 'Second paragraph' in result.output
    ...
    def test_cli_chained_operations(self):
        ...
        result = self.runner.invoke(main, ['--no-color', '.content | p | text'], input=SIMPLE_HTML)
        assert result.exit_code == 0
        assert 'First paragraph' in result.output
        assert 'Second paragraph' in result.output
    ...
    def test_cli_json_output(self):
        ...
        result = self.runner.invoke(main, ['--no-color', '--output', 'json', 'title'], input=SIMPLE_HTML)
        assert result.exit_code == 0
    ...
    def test_cli_compact_output(self):
        ...
        result = self.runner.invoke(main, ['--no-color', '--compact', '--output', 'json', 'title'], input=SIMPLE_HTML)
        assert result.exit_code == 0
        
        # Compact JSON should have no extra whitespace
        assert ': ' not in result.output or result.output.count(': ') < result.output.count(':')
    ...
    def test_cli_attribute_selector(self):
        ...
        result = self.runner.invoke(main, ['--no-color', 'input[type=text]'], input=SIMPLE_HTML)
        assert result.exit_code == 0
    ...
    def test_cli_indexing(self):
        ...
        result = self.runner.invoke(main, ['--raw', 'p | [0] | text'], input=SIMPLE_HTML)
        assert result.exit_code == 0
