...
class QueryEngine:
    ...
    def _execute_single_query(self, element: Any, query: str) -> Any:
    ...
    def _get_text(self, element: Any) -> Union[str, List[str]]:
        ...
        if isinstance(element, list):
            return [self._get_text(el) for el in element]
    ...
    def _get_html(self, element: Any) -> Union[str, List[str]]:
        """Get HTML content"""
        if isinstance(element, list):
            return [self._get_html(el) for el in element]
        elif isinstance(element, (Tag, BeautifulSoup)):
            return str(element)
        else:
            return str(element)
    ...
    def _get_by_class(self, element: Any, class_name: str) -> List[Tag]:
        ...
        if isinstance(element, list):
            ...
            return element.find_all(class_=class_name)
        else:
            return []
    ...
    def _get_by_id(self, element: Any, id_name: str) -> List[Tag]:
        ...
        if isinstance(element, list):
            results = []
            for el in element:
                found = self._get_by_id(el, id_name)
    ...
    def _get_by_tag(self, element: Any, tag_name: str) -> List[Tag]:
        ...
        if isinstance(element, list):
            ...
            return element.find_all(tag_name)
        else:
            return []
    ...
    def _get_by_attribute_value(self, element: Any, attr_query: str) -> List[Tag]:
        ...
        if isinstance(element, list):
            ...
            return element.find_all(lambda tag: tag.get(attr_name) == attr_value)
        else:
            return []
