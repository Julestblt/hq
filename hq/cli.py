...
@click.option('--color/--no-color', default=True, help='Enable/disable syntax highlighting')
@click.option('--output', '-o', type=click.Choice(['json', 'text', 'html']), 
...
def main(query: str, raw: bool, compact: bool, color: bool, output: str) -> None:
    ...
    Examples:
        ...
        curl http://example.com | hq '.container p'       # Get paragraphs in container class
        curl http://example.com | hq '#main .content'     # Get elements by ID and class
        curl http://example.com | hq 'div[id=content]'    # Get div with specific attribute
    ...
    try:
        # Read from stdin
        if sys.stdin.isatty():
            click.echo("Error: No data provided. Use a pipe or redirect input.", err=True)
            sys.exit(1)
        ...
        if not query:
            ...
            if color and not raw and output == 'html':
                syntax = Syntax(formatted_output, "html", theme="monokai", line_numbers=False)
                console.print(syntax)
