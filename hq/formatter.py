...
class OutputFormatter:
    ...
    def __init__(self, color: bool = True, compact: bool = False, raw: bool = False) -> None:
        self.color = color
    ...
    def _format_json(self, data: Any) -> str:
        ...
        if self.compact:
            return json.dumps(serializable_data, ensure_ascii=False, separators=(',', ':'))
        else:
        ...
        if isinstance(data, str):
            ...
            for item in data:
                if isinstance(item, (Tag, BeautifulSoup)):
                    ...
                    formatted = str(item.prettify()) if hasattr(item, 'prettify') else str(item)
    ...
    def _make_serializable(self, data: Any) -> Any:
        ...
        if isinstance(data, (str, int, float, bool, type(None))):
            ...
            result = {
                ...
                'text': data.get_text(strip=True) if data.get_text(strip=True) else None
            }
            
            # Add children if they exist
            children = [child for child in data.children if isinstance(child, Tag)]
            if children:
                result['children'] = [self._make_serializable(child) for child in children]
